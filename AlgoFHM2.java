...
public class AlgoFHM2 {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for (int i = 0; i < revisedTransaction.size(); i++) {
					...
					// transaction - add position in utilitylist as pid
					Element element = new Element(tid, pair.utility, remainingUtility, utilityListOfItem.elements.size());
	...
	private UtilityListFHM construct(UtilityListFHM P, UtilityListFHM px, UtilityListFHM py, int minUtility) {
		...
		for (int z = 0; z < px.elements.size(); z++) {
			...
			// Use the element's tid to find the corresponding element in py
			...
			if (P == null) {
				// Create the new element with pid as the position in the new utility list
				Element eXY = new Element(ex.tid, ex.iutils + ey.iutils, ey.rutils, pxyUL.elements.size());
				...
				if (e != null) {
					// Create new element with pid as the position in the new utility list
					Element eXY = new Element(ex.tid, ex.iutils + ey.iutils - e.iutils, ey.rutils, pxyUL.elements.size());
					// add the new element to the utility list of pXY
					pxyUL.addElement(eXY);
				}
			}
	...
	public void printStats() throws IOException {
		System.out.println("=============  FHM ALGORITHM WITH INDEX LIST ON ELEMENT - STATS =============");
 ...
 * Modified to include a pid attribute as the index list.
 */
class Element {
	/** transaction id */
	final int tid;
	...
	/** remaining utility */
	final int rutils;
	/** position in the utility list (index list) */
	final int pid;
	 ...
	 * @param pid the position id in the utility list (index list)
	 */
	public Element(int tid, int iutils, int rutils, int pid) {
		this.tid = tid;
		this.iutils = iutils;
		this.rutils = rutils;
		this.pid = pid;
...
// * Modified to use Element's pid as an index attribute.
// */
class UtilityListFHM {
	...
	Map<Integer, Integer> tidToPosition = new HashMap<>();
	...
	public void addElement(Element element) {
		...
		// Store mapping from tid to position for O(1) lookup
		tidToPosition.put(element.tid, element.pid);
	...
	public Element getElementWithTID(int tid) {
		Integer position = tidToPosition.get(tid);
		if (position == null) {
			return null;
		}

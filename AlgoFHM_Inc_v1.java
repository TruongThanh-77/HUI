...
public class AlgoFHM_Inc {
    ...
    // Construct method using hash map lookup - Phương thức xây dựng sử dụng tra cứu bản đồ băm
    private UtilityListFHM construct(UtilityListFHM P, UtilityListFHM px, UtilityListFHM py, int minUtility) {
        ...
        // Use hash map lookup - this is correct and still efficient
        for (int i = 0; i < px.elements.size(); i++) {
            ElementAlgo3 ex = px.elements.get(i);
            
            // Use hash map lookup to find element with same tid in py
            ElementAlgo3 ey = py.tidToElementMap.get(ex.tid);
            
            if (ey == null) {
                // Element not found in py
                if (ENABLE_LA_PRUNE) {
            ...
            if (P == null) {
                ...
                // Use hash map lookup to find element with same tid in P
                ElementAlgo3 e = P.tidToElementMap.get(ex.tid);
                if (e != null) {
    ...
    // Hàm findElementWithTID không còn cần thiết --> XÓA
